...
It must be highlighted that though the Controller (openbach-conductor) shall be able to process itself some of the “openbach-functions”
...
(configure/launch a Job instances, send updated information/status, etc.). For example, as explained latter, the Controller does
not schedule the Job instances itself, instead, it commands the Agent to perform the scheduling of Jobs instances. The way the backend
...
As it can be seen in Figure <<img-interf_design>>, where the main interfaces are displayed and listed, HTTP shall be used for communication between most of the elements,
...
An overall architecture of the auditorium,  the controller and the collector and their interfaces is shown in Figure <<img-controller_collector>>, where we can observe that the main streams
...
A variant of the functional scheme of OpenBACH that is used for performing operations on the collected data via the post-processing jobs is shown in Figure 18 (highlighted
in red), where:
* After a user choses to launch a post-processing job (the same way any other Job is launched)
* The Job instance shall pull the required data from the statistics database (InfluxDB) of the Collector (via the HTTP API), and then it shall perform the calculations
and push the new data the same way the Job send data to the Collector. In that case, the implementation of the Job shall contain the lib to access the database.
* Meanwhile, the user has opened the web interface of post-processing data, and the frontend has been performing get requests to obtain the data for display (still not
available). Once the post-processing Job instance pushes the new data, the web interfaces is capable of displaying it in the web browser.

Two constraints must be taken into account for correctly pushing the post-processed data into the Collector:
* The post-processed data and the original data shall not have the same name, in order to avoid deleting the orginal data.
* The post-processed shall be time stamped: sometimes with the same time stamp of the original data, but it could also be time stamped with a different time (depending on the user needs).

...
it also says the status-manager to check the status of the associated job instance. When that status of the Job instances is "Not Running", the status-manager sends this notification
to the conductor, so that it can launch other tasks (if these were depending on the first job instance).

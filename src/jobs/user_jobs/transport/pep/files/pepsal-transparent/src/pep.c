...
static void pep_proxy_data(struct pep_endpoint *from, struct pep_endpoint *to)
{
    ...
    //toip(ipbuf,from->addr);
    //toip(ipbuf1,to->addr);
    //PEP_DEBUG("# Writing data from %s to %s",ipbuf,ipbuf1); //
...
static int save_proxy_from_socket(int sockfd, struct sockaddr_in cliaddr)
{
    ...
    if(getsockname(sockfd, &orig_dst, &addrlen)
	< 0){
...
void *listener_loop(void  __attribute__((unused)) *unused)
{
	...
	/* 
	 * If the proxy is not on the table, we add the entry.
	 */
	...
	// Set don't route option
    	ret = setsockopt(out_fd, SOL_SOCKET, SO_DONTROUTE,
        	             &optval, sizeof(optval));
    	if (ret < 0) {
            pep_error("Failed to set IP_TRANSPARENT option! [RET = %d]", ret);
    	}
	...
	// Set mark
    	ret = setsockopt(out_fd, SOL_SOCKET, SO_MARK,
        	             &optval, sizeof(optval));
    	if (ret < 0) {
            pep_error("Failed to set SO_MARK option! [RET = %d]", ret);
    	}*/
	...
	// Bind to interface
 	/*	
	char *devname = "eth1";
	ret = setsockopt(out_fd, SOL_SOCKET, SO_BINDTODEVICE,
	...
	//			(void *)&ifr, sizeof(ifr));	
    	if (ret < 0) {
            pep_error("Failed to bind to device! [RET = %d]", ret);
    	}*/
        ...
        PEP_DEBUG("# Binding socket [%s:%d] --> [%s:%d]",ipbuf,proxy->src.port,ipbuf1,proxy->dst.port); //

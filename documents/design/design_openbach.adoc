...
They cannot be accessed from the Backend and they can only called by the same conductor.
...
In the tables below, we have added a column in order to show if the openbach-actions have also an associated openbach-function.
...
[frame="topbot",options="header", cols="^.^s,^.^,.^e,.^,.^"]
|===
| Action           | Method          | url              | Input contents (JSON or Query string)      | Description                     | openbach-function?
...
And finally, the group 8 of openbach-actions allowing to manage projects.
